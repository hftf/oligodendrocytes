...
END {
	# header
	printf "Packet    ";
	for (i=1; i<=num_questions_per_packet; i++) {
		if (i % 10 == 1 && i > 1) printf DELIM;
		printf "%-3s", i;
	}
	...
	for (packet in packets) {
		...
		for (qq in question_types) {
			...
			for (number=1; number<=num_questions_per_packet; number++) {
				...
				if (number % 10 == 1 && number > 1) printf DELIM;
	...
	num_questions_per_packet_nontb = 20;
	print "";
